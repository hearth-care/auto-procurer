...
- Verification so far: `uv run pytest -q` -> 166 passed; `uv run ruff check .` -> All checks passed; `uv run ruff format --check .` -> 133 files already formatted; `uv run mypy src` -> Success; `docker build -t xsource-cloud-run-smoke .` -> built successfully; `docker run --rm xsource-cloud-run-smoke xsource --help` -> exit 0; `pre-commit run --all-files` -> not runnable because `.pre-commit-config.yaml` is not present in this repo.
